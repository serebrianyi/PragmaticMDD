...
describe 'm2m for PojoWithAutoupdate', () ->
  ...
  describe 'transformClasses()', () ->
    it 'should copy header information about a class', () ->
      inputModel = { classes: [ { name: "name", attributes: []} ]}
      outputModel = {classes: []}
    ...
    it 'should copy attributes of primitive type', () ->
      inputModel = { classes: [ { attributes: [ { name: "name", type: "int" } ]} ]}
      outputModel = {classes: []}
      m2m.transformClasses(inputModel, outputModel)
  ...
  describe 'transformAssociations()', () ->
    it 'should copy association ends to the classes', () ->
      ...
      assert.equal outputModel.classes[0].associations[0].otherName, "targetName"
      assert.equal outputModel.classes[0].associations[0].otherType, "class1"
      assert.deepEqual outputModel.classes[0].associations[0].otherMultiplicity, ["0", "12"]      
      ...
      assert.equal outputModel.classes[1].associations[0].otherName, "sourceName"
      assert.equal outputModel.classes[1].associations[0].otherType, "class2"
      assert.deepEqual outputModel.classes[1].associations[0].otherMultiplicity, ["0", "1"]     