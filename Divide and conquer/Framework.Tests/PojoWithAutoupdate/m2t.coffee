...
describe 'm2t for PojoWithAutoupdate', () ->
  ...
  describe 'fieldInitializer()', () ->
    it 'should return a list for * multiplicity', () ->
      assert.equal m2t.fieldInitializer({ multiplicity: ["0", "*"]}), " = new java.util.ArrayList<>()"
  ...
  describe 'resetSetterOtherEnd()', () ->
    it 'should remove item if multiplicity is not 1', () ->
      assert.equal m2t.resetSetterOtherEnd({ otherMultiplicity: ["0", "*"], otherName: "otherName"}, "name"), "name.getOtherName().remove(this);"
    it 'should set item to null if multiplcity is 1', () ->
      assert.equal m2t.resetSetterOtherEnd({ otherMultiplicity: ["0", "1"], otherName: "otherName"}, "name"), "name.setOtherNameDirect(null);"

  describe 'setSetterOtherEnd()', () ->
    it 'should add item if multiplicity is not 1', () ->
      assert.equal m2t.setSetterOtherEnd({ otherMultiplicity: ["0", "*"], otherName: "otherName"}), "item.getOtherName().add(this);"
    it 'should set item to this if multiplcity is 1', () ->
      assert.equal m2t.setSetterOtherEnd({ otherMultiplicity: ["0", "1"], otherName: "otherName"}), "item.setOtherNameDirect(this);"

  describe 'bidirectionalHelpers()', () ->
    it 'should add bidirectional add/remove helper methods if multiplicity is not 1', () ->
      assert.include m2t.bidirectionalHelpers({ multiplicity: ["0", "*"], otherMultiplicity: ["0", "1"], name: "name", otherName: "otherName" }), "public void addNameItem"
      assert.include m2t.bidirectionalHelpers({ multiplicity: ["0", "*"], otherMultiplicity: ["0", "1"], name: "name", otherName: "otherName", type:"Class1" }), "public boolean removeNameItem"
    it 'should add bidirectional set helper methods if multiplicity is 1', () ->
      assert.include m2t.bidirectionalHelpers({ multiplicity: ["0", "1"], otherMultiplicity: ["0", "1"], name: "name", otherName: "otherName", type:"Class1" }), "public void setName"
